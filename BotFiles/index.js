...
function checkInstances() {
  ...
  if (instances.length >= 1) {
    ...
    console.log(
      chalk.red(
        boxen("FATAL ERROR: BOT ALREADY RUNNING", {
          padding: 1,
          borderStyle: "double",
        }),
      ),
...
const IgnoredRoles = process.env.IGNORED_ROLES
  ? process.env.IGNORED_ROLES.split(",")
  : [];
...
client.once(Events.ClientReady, () => {
  ...
  logStatus("success", "Autoresponse-Bot initialized (Using OCR for Images).");
...
client.on("messageCreate", async (message) => {
  ...
  if (message.attachments.size > 0) {
    logStatus(
      "ocr",
      `Analyzing ${message.attachments.size} image(s) from ${message.author.tag}...`,
    );
  ...
  for (const rule of AutoResponseRules) {
    try {
      ...
      if (response) {
        logStatus(
          "info",
          `${chalk.yellow(message.author.tag)} triggered Auto-Response in ${chalk.blue("#" + message.channel.name)}`,
        );
